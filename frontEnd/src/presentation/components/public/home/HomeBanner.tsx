...
import { useRouter } from 'next/navigation'; // Changed to standard next/navigation
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-left duration-700">
            ...
            <h1 className="text-5xl lg:text-7xl font-black text-slate-900 leading-tight">
              {t.banner.title} <br />
              <span className="text-blue-600 italic">{t.banner.titleAccent}</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button 
                className="flex items-center justify-center gap-2 bg-slate-900 text-white px-8 py-4 rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg hover:shadow-slate-200 active:scale-95"
              ...
              <button 
                className="flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-700 px-8 py-4 rounded-xl font-bold hover:bg-slate-50 transition-all active:scale-95"
                onClick={() => router.push("/shop/best-sellers")}
              >
                {t.banner.viewBestSellers}
                <ArrowRight size={20} />
              </button>
            </div>
            ...
            <div className="pt-8 flex items-center gap-8 border-t border-slate-200">
              <div>
                ...
                <p className="text-xs text-slate-500 uppercase font-semibold">{t.banner.statBooks}</p>
              </div>
              <div className="w-px h-10 bg-slate-200" />
              <div>
                <p className="text-2xl font-bold text-slate-900">12k+</p>
                <p className="text-xs text-slate-500 uppercase font-semibold">{t.banner.statReaders}</p>
              </div>
              <div className="w-px h-10 bg-slate-200" />
              <div>
                <p className="text-2xl font-bold text-slate-900">4.9/5</p>
                <p className="text-xs text-slate-500 uppercase font-semibold">{t.banner.statRating}</p>
              </div>
            </div>
          </div>
          ...
          <div className="relative hidden lg:block animate-in fade-in zoom-in duration-1000 delay-200">
            <div className="relative z-20 w-[400px] h-[550px] mx-auto group">
                ...
                <div className="absolute -bottom-6 -left-6 bg-white p-4 rounded-2xl shadow-xl border border-slate-100 flex items-center gap-3 animate-bounce">
                   ...
                   <div>
                     <p className="text-xs text-slate-500 font-medium">{t.banner.bookOfWeek}</p>
                     <p className="text-sm font-bold text-slate-900">{t.banner.featuredTitle}</p>
                   </div>
                </div>
            </div>
